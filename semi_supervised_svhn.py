...
def show_result(batch_res, fname, grid_size=(8, 8), grid_pad=5):
    if not os.path.exists("out"):
        os.mkdir("out")
    if not os.path.exists(os.path.join("out", "svhn")):
        os.mkdir(os.path.join("out", "svhn"))
    ...
    for i, res in enumerate(batch_res):
        ...
        folder_path = os.path.join(config.save_dir, 'svhn')
...
m.train(sess, config, data, summary=config.summary_n, hooks = [hook1])
