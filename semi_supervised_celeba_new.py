...
def show_result(batch_res, fname, grid_size=(8, 8), grid_pad=5):
    if not os.path.exists("out"):
        os.mkdir("out")
    if not os.path.exists(os.path.join("out", "celeba")):
        os.mkdir(os.path.join("out", "celeba"))
    ...
    for i, res in enumerate(batch_res):
        ...
        folder_path = os.path.join(config.save_dir, 'celeba')
...
def DCGANdiscriminator(x, scope="discriminator", isTrain=True): 
    ...
    with tf.variable_scope(scope):
        ...
        linear2 = fc(linear1, 17, activation_fn=None, scope='l2', reuse=tf.AUTO_REUSE)
...
celeba_train_images = np.load('celeba/celeba.train.images.npy')
celeba_test_images = np.load('celeba/celeba.test.images.npy')
...
"""
train_x, test_x = celeba_train_images, celeba_test_images
